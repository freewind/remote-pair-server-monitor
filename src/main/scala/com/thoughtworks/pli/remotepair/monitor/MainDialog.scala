import com.thoughtworks.pli.remotepair.monitor.models.Projects
...
////
////  def handleEvent(event: PairEvent): Unit = event match {
...
////    case MonitorEvent(projectName, realEventMessage, timestamp) => parseEvent(realEventMessage) match {
////      case event: CreateDocumentConfirmation => handleCreateDocumentConfirmation(projectName, event)
////      case event: ChangeContentConfirmation => handleChangeContentConfirmation(projectName, event, timestamp)
